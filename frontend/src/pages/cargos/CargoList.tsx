...
import { Plus, Edit, Eye, Trash2, Briefcase, Search } from 'lucide-react';
...
const CargoList: React.FC = () => {
  ...
  const loadCargos = useCallback(async (isSearching = false) => {
    if (!empresaId) return;
  ...
  useEffect(() => {
    loadCargos(!!debouncedSearchTerm);
  }, [loadCargos]);
  ...
  const handleDelete = async (cargo: Role) => {
    if (!empresaId) return;
    
    if (window.confirm(`Tem certeza que deseja excluir o cargo "${cargo.nome}"?`)) {
      try {
        await roleService.delete(parseInt(empresaId), cargo.roleId);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Gerencie os cargos e permissões da empresa</p>
        </div>
        <button
          onClick={() => navigate(`/empresas/${empresaId}/cargos/novo`)}
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {cargos.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-6 text-center text-gray-500">
            {debouncedSearchTerm ? 'Nenhum cargo encontrado' : 'Nenhum cargo cadastrado'}
          </div>
        ) : (
          <>
                    ...
                    <tr key={cargo.roleId}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div className="flex gap-2">
                          <button
                            onClick={() => navigate(`/empresas/${empresaId}/cargos/${cargo.roleId}`)}
                          ...
                          <button
                            onClick={() => navigate(`/empresas/${empresaId}/cargos/${cargo.roleId}/editar`)}
